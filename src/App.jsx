...
function App() {
  ...
  const calculateHealthScore = () => {
    ...
    // 营养均衡评分 (30分)
    const proteinRatio =
      nutrition.protein / Math.max(nutrition.calories / 4, 1);
    ...
    return {
      ...
      breakdown: {
        ...
        meals: {
          score:
            nutrition.mealCount >= 3
              ? 15
              : nutrition.mealCount >= 2
                ? 10
                : nutrition.mealCount >= 1
                  ? 5
                  : 0,
          max: 15,
        },
        ...
        calories: {
          score:
            caloriePercentage >= 90 && caloriePercentage <= 110
              ? 20
              : caloriePercentage >= 80 && caloriePercentage <= 120
                ? 15
                : caloriePercentage >= 70 && caloriePercentage <= 130
                  ? 10
                  : 0,
          max: 20,
        },
        ...
        {showDefensePanel && (
          <div className="defense-systems-panel">
            {Object.entries(DEFENSE_SYSTEMS).map(([key, sys]) => {
              const isCovered = foods.some((f) =>
                f.healthData?.systems?.includes(key),
              );
              return (
                <div
                  key={key}
        ...
        {showAnalytics && (
          <div className="analytics-content">
                  ...
                  <div className="profile-stats">
                    ...
                    <div className="profile-stat">
                      <span className="stat-icon">
                        {getBMICategory().emoji}
                      </span>
                      <div className="stat-info">
                        <span
                          className="stat-value"
                          style={{ color: getBMICategory().color }}
                        >
                          {calculateBMI()}
                        </span>
                        <span className="stat-label">
                          BMI {getBMICategory().category}
                        </span>
                      </div>
                    </div>
                    <div className="profile-stat">
                    ...
                    <div className="diet-pattern-card">
                      ...
                      <div className="diet-tags">
                        {nutrition.detectedDiets.map((diet, idx) => (
                          <div key={idx} className="diet-tag">
                            <span className="diet-emoji">
                              {DIET_PATTERNS[diet.pattern].emoji}
                            </span>
                            <div className="diet-info">
                              <span className="diet-name">
                                {DIET_PATTERNS[diet.pattern].name}
                              </span>
                              <span className="diet-description">
                                {DIET_PATTERNS[diet.pattern].description}
                              </span>
                              <span className="diet-match">
                                匹配度: {diet.percentage}%
                              </span>
                            </div>
                          </div>
                      ...
                      {nutrition.detectedDiets.length > 0 && (
                        <p className="diet-benefits">
                          💡 好处:{" "}
                          {nutrition.detectedDiets
                            .map((d) =>
                              DIET_PATTERNS[d.pattern].benefits.join("、"),
                            )
                            .join(" | ")}
                        </p>
                      )}
            ...
            <div className="health-goal-card">
              <h4>
                {getHealthGoalRecommendations().emoji} 健康目标:{" "}
                {getHealthGoalRecommendations().name}
              </h4>
              <p className="goal-focus">
                重点: {getHealthGoalRecommendations().focus}
              </p>
              ...
              <div className="goal-targets">
                <div className="target-item">
                  ...
                  <span className="target-value">
                    {getHealthGoalRecommendations().targetCalories} 千卡
                  </span>
                </div>
                <div className="target-item">
                  <span className="target-label">目标蛋白质</span>
                  <span className="target-value">
                    {getHealthGoalRecommendations().targetProtein}g
                  </span>
                </div>
              </div>
              <button
